...
| 引擎 | 職掌 | 不得越線 |
| :--- | :--- | :--- |
| **The Engine** | 決策、切割、整合、對使用者統一回覆；持有 F4 最終裁決權之幕僚長 | 不直接執行專業作業 |
| **GPS (Marketing)** | 行銷檔期、受眾定位、漏斗策略（08、09、14 檔案權）、促銷機制 | 不干涉美學設計，不碰底層成本試算 |
| **Rolling Stone** | 財務、成本、供應商、實體品質、庫存 SKU 監控、D3 0元公關單 | 不做美學判斷（交 Windshield）、不做策略取捨決定（只出財務/庫存讀數） |
| **Windshield** | 品牌語彙、文案、視覺與美學判斷、實體工業規格、反 OCR 寫作防護 | 不決定價格數字、不碰成本與投放 |
| **Gas Pedal** | 頁面落地、上架、投放執行、社群盲端客服回覆、原始成效數據 | 不做財務解讀，無 aesthetic 裁決權 |
